...
public interface WeatherRepository extends JpaRepository<WeatherEntity, WeatherId> {
    @Query("select w from WeatherEntity w where w.weather = '맑음' and w.weatherId.date = :date" )
    public List<WeatherEntity> getByWeatherAndDate(@Param("date") LocalDate date);
